...
{
    //
    public function edit_stressecho(Request $request)
    {   
        try {
            $id = $_GET['id'];
            $exam = StressEcho::select(
                'exam_stressecho.*',
                'tran_admission.patientcode as patientcode'
            )
                ->where('exam_stressecho.admission_id', $id)
                ->leftJoin(
                    'tran_admission',
                    'tran_admission.id',
                    'exam_stressecho.admission_id'
                )
                ->latest('id')
                ->first();
            ...
            ->get();
            return view('StressEcho.edit-stressecho', compact('exam', 'patient', 'admission', 'cardiologists'));
    ...
    public function update_stressecho(Request $request)
    {   
        try {
            $id = $request->id;
            $exam = StressEcho::findOrFail($id);
            ...
            $log->description =
                'Update Stress Echo from Patient ' . $request->patientcode;
            $log->date = date('Y-m-d');
            $log->save();
    
            if ($save) {
                return back()->with('status', 'Stress Echo updated!!');
            }
        } catch (\Exception $exception) {
            $message = $exception->getMessage();
            $file = $exception->getFile();
    ...
    public function add_stressecho(Request $request)
    {   
        try {
            $id = $_GET['id'];
            ...
            ->leftJoin('mast_employeeinfo', 'mast_employee.id', 'mast_employeeinfo.main_id')
            ->get();
        return view('StressEcho.add-stressecho', compact('admission', 'cardiologists'));
        } catch (\Exception $exception) {
            $message = $exception->getMessage();
            $file = $exception->getFile();
            return view('errors.error', compact('message', 'file'));
    ...
    public function store_stressecho(Request $request)
    {   
        try {
            $exam = new StressEcho();
            ...
            $log->employee_id = $employeeInfo['employeeId'];
            $log->description =
                'Add Stress Echo from Patient ' . $request->patientcode;
            $log->date = date('Y-m-d');
            $log->save();
            ...
            $path =
                ...
                $request->patientcode;
            if ($save) {
                return redirect($path)->with('status', 'Stress Echo added!!')->with('redirect', 'basic-exam;child-basic-tab;child-basic-component;baseVerticalLeft1-tab12;tabVerticalLeft12');
            }
        } catch (\Exception $exception) {
            $message = $exception->getMessage();
            $file = $exception->getFile();
